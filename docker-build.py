...
release = environ['GLUON_RELEASE_DOCKER_ENV']
branch = environ['GLUON_TAG_DOCKER_ENV']
broken = environ['GLUON_BROKEN_DOCKER_ENV']
...
with open('%s/git.log' % logdir, 'w') as log:
    check_call('git clone https://github.com/freifunk-gluon/gluon.git %s -b "%s"' % (gluondir, branch), stdout=log, stderr=log, shell=True)
...
copytree(domainsdir, '%s/domains' % sitedir)
...
if 'GLUON_TARGETS_DOCKER_ENV' in environ:
    targets = environ['GLUON_TARGETS_DOCKER_ENV'].split()
else:
...
for target in targets:
    ...
    if rc == 0:
        ...
        modulesbuilddir = 'output/packages/gluon-%s/%s/%s' % (release, arch, variant)
        if exists(modulesbuilddir):
            ...
            rename('output/packages/gluon-%s/%s/%s' % (release, arch, variant), variantdir)
